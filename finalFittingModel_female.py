...
def plotIndFits(params, lunarDays, cpue, sizes, error):
    ...
    plt.xlabel('Days from Full Moon')
    plt.ylabel('Seasonal Adjusted CPUE')
    plt.legend()
    plt.show()
    fig2.savefig("FemaleSquid.tiff")
...
def main():
    ...
    cpue = list(np.array(totalData)[1:,29])
    raw_weights = list(np.array(totalData)[1:,15])
    ...
    siz=list(10*np.array(raw_weights))


    x0 = np.array([0.5, 2, 0.05])  # initial guess for curve fit parameters
    ...
    res = minimize(hill, x0, args=(lunarIndex, cpue, norm_weights), method='SLSQP',
                   bounds=((0, None), (0, 29), (-5, 5)), tol=1e-6)
